...
async fn main() {
    ...
    let upstream = std::env::var("UPSTREAM_URL")
        .unwrap_or_else(|_| "http://localhost:8138".to_string());
    ...
    let port = std::env::var("GATEWAY_PORT").unwrap_or_else(|_| "9138".to_string());
    let addr = format!("0.0.0.0:{}", port);
    info!("alice-ble-gateway listening on {}", addr);
